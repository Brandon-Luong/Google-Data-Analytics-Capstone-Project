...
library(patchwork) #ggplots helper
```
...
```{r}
# Drop tracker_distance, logged_activities_distance, sedentary_active_distance
merged_df <- merged_df %>% 
  select(-c(
    tracker_distance, 
...
Because most people often talk about how many hours they have slept as opposed to how many minutes, I will add new columns for sleep hours.
...
merged_df <- merged_df %>% 
  mutate(
    ...
    hours_in_bed = total_time_in_bed / 60)
...
merged_df %>% 
  group_by(day_of_week) %>% 
  summarise(average_distance = mean(total_distance))
```
...
merged_df %>% 
  group_by(day_of_week) %>% 
  summarise(average_sleep_hours = mean(hours_asleep))
```

Interestingly enough, Wednesday has the 2nd most hours slept on average. This could maybe be accredited to Wednesday's "hump day" in which people tend to feel tired and sluggish during the middle of the work week.

## Plots

Let's plot total_steps, total_distance, and calories vs day_of_week so that the data is viewed visually.

```{r}
ggplot(data = merged_df) + 
  geom_boxplot(mapping = aes(x=day_of_week, y=total_steps, color=day_of_week), notch=TRUE) + 
  scale_y_continuous(n.breaks = 10)
```

```{r}
ggplot(data = merged_df) + 
  geom_boxplot(mapping = aes(x=day_of_week, y=total_distance, color=day_of_week), notch=TRUE) + 
  scale_y_continuous(n.breaks = 10)
```
...
Let's take a look at hours_asleep vs day_of_week.

```{r}
merged_df %>% 
  ggplot() + 
  geom_boxplot(mapping = aes(x=day_of_week, y=hours_asleep, color=day_of_week)) + 
  scale_y_continuous(n.breaks = 6)
```

As expected, the average hours of sleep per day is about 7 hours, except for Sunday for which its average is 8 hours (though a 1 hour difference is not statistically significant). This plot further supports the idea of "lazy Sundays." Saturday and Sunday also have the largest range of values (ie. largest boxes) which can correspond with people's weekend plans which can vary widely between users. Contrasting this with Monday to Friday, the "work days," in which the boxes are much smaller meaning they mostly slept on a consistent schedule.

Let's take a look at hours_asleep vs hours_in_bed.

```{r}
ggplot(data = merged_df) +
  geom_point(mapping = aes(x=hours_in_bed, y=hours_asleep)) + 
  geom_smooth(mapping = aes(x=hours_in_bed, y=hours_asleep)) +
  scale_x_continuous(n.breaks = 10) +
  scale_y_continuous(n.breaks = 8)
```

Overall, this plot is consistent with the fact that most of the time spent asleep is the same as the time spent in bed. There are a couple of outliers in which some users were unable to sleep causing them to have a lower hours_asleep to hours_in_bed ratio.
...
Comparing calories vs total_steps, there seems to be a general trend that more steps means more calories burned. But the range of values for each step value is quite large (eg. at 10,000 total steps, calories burned ranges from 1500 to 3500), so much so that the best fit line is a little misleading and inaccurate.

Let's compare the intensities (very_active_distance, moderately_active_distance, light_active_distance) vs calories.

```{r}
very_active_dist_plot <- ggplot(data = merged_df) + 
  geom_point(mapping = aes(x=very_active_distance, y=calories)) + 
  geom_smooth(mapping = aes(x=very_active_distance, y=calories))

moderately_active_dist_plot <- ggplot(data = merged_df) + 
  geom_point(mapping = aes(x=moderately_active_distance, y=calories)) +
  geom_smooth(mapping = aes(x=moderately_active_distance, y=calories))

light_active_dist_plot <- ggplot(data = merged_df) + 
  geom_point(mapping = aes(x=light_active_distance, y=calories)) + 
  geom_smooth(mapping = aes(x=light_active_distance, y=calories))

(very_active_dist_plot | moderately_active_dist_plot) / light_active_dist_plot
```
