...
```
It looks like tracker_distance has the same values as total_distance except for 3 rows, of which come from the same user. I think I can safely drop tracker_distance and use total_distance instead.

```{r}
# logged_activities_distance values
...
So it looks like the majority of logged_activities_distance values were never recorded. The only values, other than 0, are from 2 users. Additionally, I'm not sure what logged_activities_distance represents, as it is different from total_distance and from (very, moderately, light, sedentary) active distance.

With no context and the majority of values 0, I will drop this column as well.

```{r}
# Drop tracker_distance and logged_activities_distance
merged_df <- merged_df %>% 
