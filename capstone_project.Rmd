...
```{r}
# Average steps taken on a given day
merged_df %>% 
...
```{r}
only_variables <- merged_df %>% 
  select(4:18)

round(cor(only_variables), 2)
```
...
```{r}
table(merged_df$day_of_week)
```

```{r}
merged_df %>% 
  group_by(id) %>% 
  summarise(day_freq = n_distinct(day_of_week))
```

```{r}
ggplot(data=merged_df) + 
  geom_point(mapping = aes(x=day_of_week, y=total_steps))
```
